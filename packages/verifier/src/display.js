...
export function renderExpandedDetails(result) {
  ...
  const rows = [
    ...
    ["Issuer trust source", result.checks.issuer_trust_source ?? "unknown"],
    ["Trust input class", result.checks.trust_input_class ?? "unknown"],
    ["Trust bundle id", result.checks.trust_bundle_id ?? "none"],
    ["Trust bundle version", result.checks.trust_bundle_version ?? "none"],
    ["Trust bundle status", result.checks.trust_bundle_status ?? "none"],
    ["Trust bundle scope", result.checks.trust_bundle_scope ?? "none"],
